...
def combineImages(name_initial, indices):
    ...
    recon = np.rollaxis(np.asarray(Image.open('recon.jpg'))[
                        padding:-padding, padding:-padding, :], 2, 0) / 255.0
    ...
    for i in range(steps):
        ...
        allt.append(origin[:, hfrom:(hfrom + width - padding),
                    wfrom:(wfrom + width - padding)])
    for i in range(steps):
        hfrom = int((i // grid_size) * recon_width)
        wfrom = int((i % grid_size) * recon_width)
        allt.append(recon[:, hfrom:(hfrom + width - padding),
                    wfrom:(wfrom + width - padding)])
    for index in indices:
        for image in images:
            allt.append(image[:, :, (index * width)                        :((index + 1) * width - padding)])
    print(allt[0].shape)
    save_image(tensor=torch.tensor(
        allt), filename='result/{}_combined.eps'.format(name_initial), nrow=steps, pad_value=1)
