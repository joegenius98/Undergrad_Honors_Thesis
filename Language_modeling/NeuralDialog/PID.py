...
class PIDControl():
    ...
    def pid(self, KL_loss, Kp=0.01, Ki=-0.0001):
        ...
        # Ik=tf_print(Ik,'Ik')
        # when time = k-1
        Ik = tf.cond(self.W_k1 < 0, lambda: self.I_k1, lambda: tf.cond(
            self.W_k1 > 1, lambda: self.I_k1, lambda: Ik))
        ...
        with tf.control_dependencies([op1, op2]):
            Wk = tf.cond(Wk > 1, lambda: 1.0, lambda: tf.cond(
                Wk < 0, lambda: 0.0, lambda: Wk))
