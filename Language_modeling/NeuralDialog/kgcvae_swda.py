...
tf.app.flags.DEFINE_string(
    "word2vec_path", './glove/glove.twitter.27B.200d.txt', "The path to word2vec. Can be None.")
tf.app.flags.DEFINE_string(
    "data_dir", "./data/full_swda_clean_42da_sentiment_dialog_corpus.p", "Raw data directory.")
...
tf.app.flags.DEFINE_string(
    "model_name", "cost_anneal", "cost annealing model.")
...
tf.app.flags.DEFINE_string("test_path", "run_cost_anneal_20000",
...
def main():
    ...
    api = SWDADialogCorpus(
        FLAGS.data_dir, word2vec=FLAGS.word2vec_path, word2vec_dim=config.embed_size)
    ...
    with tf.Session() as sess:
        initializer = tf.random_uniform_initializer(
            -1.0 * config.init_w, config.init_w)
        scope = "model"
        with tf.variable_scope(scope, reuse=None, initializer=initializer):
            model = KgRnnCVAE(sess, config, api, log_dir=None if FLAGS.forward_only else log_dir,
                              forward=False, pid_control=pid, scope=scope)
        with tf.variable_scope(scope, reuse=True, initializer=initializer):
            valid_model = KgRnnCVAE(
                sess, valid_config, api, log_dir=None, forward=False, pid_control=pid, scope=scope)
        with tf.variable_scope(scope, reuse=True, initializer=initializer):
            test_model = KgRnnCVAE(
                sess, test_config, api, log_dir=None, forward=True, pid_control=pid, scope=scope)
        ...
        if not FLAGS.forward_only:
            ...
            for epoch in range(config.max_epoch):
                ...
                global_t, train_loss = model.train(
                    global_t, sess, train_feed, update_limit=config.update_limit)
                ...
                test_feed.epoch_init(valid_config.batch_size, valid_config.backward_size,
                                     valid_config.step_size, shuffle=False, intra_shuffle=False)
                elbo, nll, ppl, au_count, kl_loss = valid_model.valid(
                    "ELBO_TEST", sess, valid_feed, test_feed)
                print('middle test nll: {} ppl: {} ActiveUnit: {} kl_loss:{}\n'.format(
                    nll, ppl, au_count, kl_loss))
            ...
            test_model.test(sess, test_feed, num_batch=None,
                            repeat=10, dest=dest_f)
            dest_f.close()
            print("****testing done****")
        else:
            # begin validation
