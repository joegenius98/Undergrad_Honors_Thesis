...
from annealing import _cyclical_annealing, _cost_annealing
        ...
        for _ in range(num_layer):
            if cell_type == "gru":
                ...
                cell = rnn_cell.LSTMCell(
                    cell_size, use_peepholes=False, forget_bias=1.0)
    ...
    def optimize(self, sess, config, loss, log_dir):
        ...
        if config.grad_clip is not None:
            grads, _ = tf.clip_by_global_norm(
                grads, tf.constant(config.grad_clip))
        # add gradient noise
        if config.grad_noise > 0:
            grad_std = tf.sqrt(config.grad_noise /
                               tf.pow(1.0 + tf.to_float(self.global_t), 0.55))
            grads = [
                g + tf.truncated_normal(tf.shape(g), mean=0.0, stddev=grad_std) for g in grads]
...
class KgRnnCVAE(BaseTFModel):
    ...
    def __init__(self, sess, config, api, log_dir, forward, pid_control, scope=None):
        ...
        # self.last_KL = tf.Variable(10.0,trainable=False)

        with tf.name_scope("io"):
            # all dialog context and known attributes
            self.input_contexts = tf.placeholder(dtype=tf.int32, shape=(
                None, None, self.max_utt_len), name="dialog_context")
            self.floors = tf.placeholder(
                dtype=tf.int32, shape=(None, None), name="floor")
            self.context_lens = tf.placeholder(
                dtype=tf.int32, shape=(None,), name="context_lens")
            self.topics = tf.placeholder(
                dtype=tf.int32, shape=(None,), name="topics")
            self.my_profile = tf.placeholder(
                dtype=tf.float32, shape=(None, 4), name="my_profile")
            self.ot_profile = tf.placeholder(
                dtype=tf.float32, shape=(None, 4), name="ot_profile")

            # target response given the dialog context
            self.output_tokens = tf.placeholder(
                dtype=tf.int32, shape=(None, None), name="output_token")
            self.output_lens = tf.placeholder(
                dtype=tf.int32, shape=(None,), name="output_lens")
            self.output_das = tf.placeholder(
                dtype=tf.int32, shape=(None,), name="output_dialog_acts")

            # optimization related variables
            self.learning_rate = tf.Variable(
                float(config.init_lr), trainable=False, name="learning_rate")
        ...
        with variable_scope.variable_scope("topicEmbedding"):
            t_embedding = tf.get_variable(
                "embedding", [self.topic_vocab_size, config.topic_embed_size], dtype=tf.float32)
        ...
        if config.use_hcf:
            with variable_scope.variable_scope("dialogActEmbedding"):
                d_embedding = tf.get_variable(
                    "embedding", [self.da_vocab_size, config.da_embed_size], dtype=tf.float32)
        ...
        with variable_scope.variable_scope("wordEmbedding"):
            self.embedding = tf.get_variable(
                "embedding", [self.vocab_size, config.embed_size], dtype=tf.float32)
            ...
            if config.sent_type == "bow":
                ...
                sent_cell = self.get_rnncell(
                    "gru", self.sent_cell_size, config.keep_prob, 1)
                input_embedding, sent_size = get_rnn_encode(
                ...
                fwd_sent_cell = self.get_rnncell(
                    "gru", self.sent_cell_size, keep_prob=1.0, num_layer=1)
                bwd_sent_cell = self.get_rnncell(
                    "gru", self.sent_cell_size, keep_prob=1.0, num_layer=1)
                input_embedding, sent_size = get_bi_rnn_encode(
                    input_embedding, fwd_sent_cell, bwd_sent_cell, scope="sent_bi_rnn")
                output_embedding, _ = get_bi_rnn_encode(
                    output_embedding, fwd_sent_cell, bwd_sent_cell, self.output_lens, scope="sent_bi_rnn", reuse=True)
            else:
                raise ValueError(
            ...
            input_embedding = tf.reshape(
                input_embedding, [-1, max_dialog_len, sent_size])
            if config.keep_prob < 1.0:
                input_embedding = tf.nn.dropout(
                    input_embedding, config.keep_prob)

            # convert floors into 1 hot
            floor_one_hot = tf.one_hot(tf.reshape(
                self.floors, [-1]), depth=2, dtype=tf.float32)
            floor_one_hot = tf.reshape(floor_one_hot, [-1, max_dialog_len, 2])

            joint_embedding = tf.concat(
                [input_embedding, floor_one_hot], 2, "joint_embedding")

        with variable_scope.variable_scope("contextRNN"):
            enc_cell = self.get_rnncell(
                config.cell_type, self.context_cell_size, keep_prob=1.0, num_layer=config.num_layer)
        ...
        if config.use_hcf:
            ...
            attribute_fc1 = layers.fully_connected(
                attribute_embedding, 30, activation_fn=tf.tanh, scope="attribute_fc1")

        cond_list = [topic_embedding, self.my_profile,
                     self.ot_profile, enc_last_state]
        ...
        with variable_scope.variable_scope("recognitionNetwork"):
            ...
            self.recog_mulogvar = recog_mulogvar = layers.fully_connected(
                recog_input, config.latent_size * 2, activation_fn=None, scope="muvar")
        ...
        with variable_scope.variable_scope("generationNetwork"):
            ...
            bow_fc1 = layers.fully_connected(
                gen_inputs, 400, activation_fn=tf.tanh, scope="bow_fc1")
            if config.keep_prob < 1.0:
                bow_fc1 = tf.nn.dropout(bow_fc1, config.keep_prob)
            self.bow_logits = layers.fully_connected(
                bow_fc1, self.vocab_size, activation_fn=None, scope="bow_project")

            # Y loss
            if config.use_hcf:
                meta_fc1 = layers.fully_connected(
                    gen_inputs, 400, activation_fn=tf.tanh, scope="meta_fc1")
                if config.keep_prob < 1.0:
                    meta_fc1 = tf.nn.dropout(meta_fc1, config.keep_prob)
                self.da_logits = layers.fully_connected(
                    meta_fc1, self.da_vocab_size, scope="da_project")
        ...
        with variable_scope.variable_scope("decoder"):
            dec_cell = self.get_rnncell(
                config.cell_type, self.dec_cell_size, config.keep_prob, config.num_layer)
            ...
            if forward:
                ...
                if config.dec_keep_prob < 1.0:
                    ...
                    dec_input_embedding = tf.reshape(
                        dec_input_embedding, [-1, max_out_len-1, config.embed_size])
            ...
            if final_context_state is not None:
                final_context_state = final_context_state[:, 0:array_ops.shape(dec_outs)[
                    1]]
        ...
        if not forward:  # training
            with variable_scope.variable_scope("loss"):
                ...
                rc_loss = tf.reduce_sum(
                    rc_loss * label_mask, reduction_indices=1)
                self.avg_rc_loss = tf.reduce_mean(rc_loss)
                # used only for perpliexty calculation. Not used for optimzation
                self.rc_ppl = tf.exp(tf.reduce_sum(
                    rc_loss) / tf.reduce_sum(label_mask))

                """ as n-trial multimodal distribution. """
                tile_bow_logits = tf.tile(tf.expand_dims(
                    self.bow_logits, 1), [1, max_out_len - 1, 1])
                ...
                kld = gaussian_kld(recog_mu, recog_logvar,
                                   prior_mu, prior_logvar)
                self.avg_kld = tf.reduce_mean(kld)

                # ---------------------------------------
                # add pid here by Huajie Shao
                # ---------------------------------------
                if log_dir is not None and FLAGS.mode == 'train':
                    # kl_weights = tf.minimum(tf.to_float(self.global_t)/config.full_kl_step, 1.0)
                    if 'cost' in FLAGS.model_name:
                        kl_weights = _cost_annealing(
                            self.global_t, FLAGS.anneal_steps)
                    elif 'pid' in FLAGS.model_name:
                        kl_weights = self.pid_control.pid(
                            self.avg_kld, FLAGS.Kp, FLAGS.Ki)
                    elif 'cyclical' in FLAGS.model_name:
                        kl_weights = _cyclical_annealing(
                            self.global_t, FLAGS.cycle)
                else:
                ...
                tf.summary.scalar("KL_weight", self.kl_w)
                ...
                self.log_p_z = norm_log_liklihood(
                    latent_sample, prior_mu, prior_logvar)
                self.log_q_z_xy = norm_log_liklihood(
                    latent_sample, recog_mu, recog_logvar)
                self.est_marginal = tf.reduce_mean(
                    rc_loss + bow_loss - self.log_p_z + self.log_q_z_xy)

            self.optimize(sess, config, aug_elbo, log_dir)

        self.saver = tf.train.Saver(
            tf.global_variables(), write_version=tf.train.SaverDef.V2)
    ...
    def train(self, global_t, sess, train_feed, update_limit=5000):
        ...
        loss_names = ["elbo_loss", "bow_loss",
                      "rc_loss", "rc_peplexity", "kl_loss"]
        while True:
            ...
            _, sum_op, elbo_loss, bow_loss, rc_loss, rc_ppl, kl_loss, kl_w = sess.run([self.train_ops, self.summary_op,
                                                                                       self.elbo, self.avg_bow_loss,
                                                                                       self.avg_rc_loss, self.rc_ppl, self.avg_kld, self.kl_w],
                                                                                      feed_dict)
            ...
            if local_t % 500 == 0:
                self.print_loss("global_t:%d ptr:%.2f" % (global_t, train_feed.ptr / float(train_feed.num_batch)),
        ...
        avg_losses = self.print_loss("Epoch Done", loss_names,
                                     [elbo_losses, bow_losses,
                                         rc_losses, rc_ppls, kl_losses],
                                     "step time %.4f" % (epoch_time / train_feed.num_batch))

        return global_t, avg_losses[0]

    def valid(self, name, sess, valid_feed, test_feed):
        elbo_losses = []
        rc_losses = []
        rc_ppls = []
        bow_losses = []
        ...
        cnt = 0
        means_sum = 0
        # test_feed = valid_feed
        while True:
            batch = valid_feed.next_batch()
            if batch is None:
                break
            feed_dict = self.batch_2_feed(
                batch, None, use_prior=False, repeat=1)
            mean, elbo_loss, bow_loss, rc_loss, rc_ppl, kl_loss = sess.run(
                [self.prior_mu, self.elbo, self.avg_bow_loss, self.avg_rc_loss,
            ...
            bow_losses.append(bow_loss)
            kl_losses.append(kl_loss)
            means_sum += np.sum(mean, axis=0, keepdims=True)
            cnt += mean.shape[0]
            # print(mean.shape)
        ...
        nll = avg_losses[2] + avg_losses[4]
        ppl = avg_losses[3]
        kl_loss = avg_losses[-1]
        # print("kl loss: ", avg_losses[4])
        # run test batch to compute active unit
        var_sum = 0
        mean_mean = means_sum/cnt
        au_count = 0
        while True:
            # print("enter here now")
            batch = test_feed.next_batch()
            if batch is None:
                break
            feed_dict = self.batch_2_feed(
                batch, None, use_prior=False, repeat=1)
            mean, elbo_loss = sess.run([self.prior_mu, self.elbo], feed_dict)
            var_sum += np.sum((mean - mean_mean) ** 2, axis=0)
            cnt += mean.shape[0]
        au_var = var_sum / (cnt - 1)
        au_var = np.asarray(au_var)
        au_count = (au_var >= 0.01).sum()

        return avg_losses[0], nll, ppl, au_count, kl_loss
    ...
    def test(self, sess, test_feed, num_batch=None, repeat=5, dest=sys.stdout):
        ...
        while True:
            ...
            if batch is None or (num_batch is not None and local_t > num_batch):
                break
            feed_dict = self.batch_2_feed(
                batch, None, use_prior=True, repeat=repeat)
            word_outs, da_logits = sess.run(
                [self.dec_out_words, self.da_logits], feed_dict)
            ...
            # print('batch_size: ', test_feed.batch_size)
            ...
            for b_id in range(test_feed.batch_size):
                ...
                t_id = true_srcs.shape[1] - 1
                src_str = " ".join(
                    [self.vocab[e] for e in true_srcs[b_id, t_id].tolist() if e != 0])
                dest.write("Src %d-%d: %s\n" %
                           (t_id, true_floor[b_id, t_id], src_str))

                # print the true outputs
                true_tokens = [self.vocab[e] for e in true_outs[b_id].tolist() if e not in [
                    0, self.eos_id, self.go_id]]
                ...
                for r_id in range(repeat):
                    ...
                    pred_tokens = [
                        self.vocab[e] for e in pred_outs[b_id].tolist() if e != self.eos_id and e != 0]
