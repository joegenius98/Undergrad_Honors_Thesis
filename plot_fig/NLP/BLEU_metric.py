...
def _compute_sentence_bleu(k, reference, predict):
    ...
    bleu3 = sentence_bleu([reference[k]], predict[k],
                          weights=[0.3333, 0.33333, 0.3333])
    bleu4 = sentence_bleu([reference[k]], predict[k],
...
def main():
    path_list = ['cost_anneal_20000-v', 'cyclical_4-v',
                 'pid17-v', 'pid25-v', 'pid35-v']
    with open('./result/BLEU.txt', "w") as fout:
        for path in path_list:
            ...
            for i in [1, 2, 3, 4]:
                ...
                bleu1, bleu2, bleu3, bleu4 = _multiple_run(
                    file_ground, file_predict)
